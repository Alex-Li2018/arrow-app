...
function parseActions (rawActions, defaultValue = []) {
...
function insert (history, state, limit, group) {
  ...
  log('inserting', state);
  log('new free: ', limit - lengthWithoutFuture);
...
function undoable (reducer, rawConfig = {}) {
  set(rawConfig.debug);
  ...
  return (state = initialState, action = {}, ...slices) => {
    ...
    if (!initialState) {
      ...
      if (state === undefined) {
        ...
        log('do not set initialState on probe actions');
        end(history);
        return history
    ...
    switch (action.type) {
      ...
      case config.jumpToPastType:
        ...
        log(`perform jumpToPast to ${action.index}`);
        end(res);
        return skipReducer(res, action, ...slices)

      case config.jumpToFutureType:
        res = jumpToFuture(history, action.index);
        log(`perform jumpToFuture to ${action.index}`);
        end(res);
        return skipReducer(res, action, ...slices)

      case config.jumpType:
        res = jump(history, action.index);
        log(`perform jump to ${action.index}`);
        end(res);
        return skipReducer(res, action, ...slices)

      case actionTypeAmongClearHistoryType(action.type, config.clearHistoryType):
        res = createHistory(history.present, config.ignoreInitialState);
        log('perform clearHistory');
        end(res);
        return skipReducer(res, action, ...slices)
      ...
      default:
        ...
        if (config.initTypes.some((actionType) => actionType === action.type)) {
          log('reset history due to init action');
          end(initialState);
          return initialState
        }
        ...
        if (filtered) {
          ...
          log('filter ignored action, not storing it in past');
          end(filteredState);
        ...
        if (group != null && group === history.group) {
          ...
          log('groupBy grouped the action with the previous action');
          end(groupedState);
          return groupedState
        }
        ...
        log('inserted new state into history');
        end(history);
        return history
    }
  }
